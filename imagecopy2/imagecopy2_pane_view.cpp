...
namespace imagecopy2
{
   ...
   void pane_view::_001OnCreate(message::message * pobj)
   {
      ...
      add_tab("Image Copy", ::imagecopy2::PaneViewImageCopy, true, true);
      add_tab("Import", "file_manager:import", true, true);
      add_tab("Export", "file_manager:export", true, true);

      set_cur_tab_by_id(::imagecopy2::PaneViewImageCopy);
   ...
   void pane_view::on_create_impact(::user::impact_creator_data * pcreatordata)
   {
      ...
      {
      case PaneViewImageCopy:
         {
            m_pdocMain=dynamic_cast < document * > (Application.m_ptemplateMain->open_document_file(var(), true, pcreatordata->m_pholder));
         }
