...
{
   m_pmaininterface->update_app(false);
}
...
- (IBAction)on_restart:(id)sender
{
   ...
   self->m_pmaininterface->start_update(m_emode);
...
void main_interface::on_radio_download_response(bool bDownloadEnable, bool bUpdateEnable)
{
   ...
   m_pmainwindow->m_emode = mode_normal;
...
void main_interface::on_radio_browse_response(bool bUpdateEnable)
{
   ...
   m_pmainwindow->m_emode = mode_browse;
...
void main_interface::on_app_check_version(const char * pszStatus)
{
   ...
   if([strStatus length] <= 0)
   {
      ...
      [[[m_pmainwindow label_app_update] dd_invokeOnMainThread] setHidden:YES];
   
      [[[m_pmainwindow button_app_update] dd_invokeOnMainThread] setHidden:YES];
...
void main_interface::on_app_update_cancellable(bool bCancellable)
{
   ...
   if (!bCancellable)
   {
      ...
      //[[m_pmainwindow window] setEnabled: NO];
