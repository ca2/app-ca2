...
namespace quasar_science_update_utility
{
   ...
   void dialog1::on_notify(int iCode, int iId)
   {
      ...
      else if (iCode == CBN_SELENDOK && iId == IDC_COMBO_LANGUAGE)
      {

         if (!System.m_pstrlangmap->set_current_language(m_comboLang.get_combo_sel()))
         {

            MessageBox(Application.m_pmainwindow->m_hwnd, _("Could not change language"), _("Error"), MB_ICONEXCLAMATION);
         ...
         else
         {
            ...
            call_async(Context.file().module(), "", "", SW_SHOW, true);

            Sleep(1000);

            PostMessage(Application.m_pmainwindow->m_hwnd, WM_CLOSE, 0, 0);
   ...
   bool dialog1::on_init_dialog()
   {
      ...
      set_dlg_item_text(IDC_DESCRIPTION, _("IDS_DESCRIPTION1"));
      ...
      get_dlg_item(m_comboLang, IDC_COMBO_LANGUAGE);

      m_comboLang.send_message(CB_RESETCONTENT);


      System.m_pstrlangmap->_001FillCombo(m_comboLang.m_hwnd);
   ...
   void dialog1::on_command(UINT nID)
   {
      ...
      else if (nID == IDC_BROWSE)
      {
         ...
         if (Context.os().browse_file_open(NULL, set))
         {
            ...
            m_editBrowse._set_window_text(set["file_name"]);
