...
namespace grbl_merge_tool
{
   ...
   string twitter::grbl_merge_tool(const char * pszMessage)
   {
   ...
   string twitter::grbl_merge_toolmator_noauth(const char * pszMessage, stringa straMediaIds)
   {
      ...
      m_strLastRequest.Format("grbl_merge_toolmator_noauth(\"%s\")", pszMessage);
...
//      string strPathKey = Context.dir().userappdata() / "twitterClient_token_key" + ::str::from(m_eversion) + ".txt";
  //    string strPathSecret = Context.dir().userappdata() / "twitterClient_token_secret" + ::str::from(m_eversion) + ".txt";
   ...
   var twitter::last_grbl_merge_toolmator(const string & strUser, bool bForceAppAuth)
   {
      ...
      m_strLastRequest.Format("last_grbl_merge_toolmator(\"%s\", %d)", strUser, (int) bForceAppAuth);
      ...
      if (!bForceAppAuth && (str.is_empty() || v["errors"].get_count() > 0))
      {
         ...
         return last_grbl_merge_toolmator(strUser, true);
   ...
   var twitter::next_grbl_merge_toolmators(const string & strUser, const string & strSince, bool bForceAppAuth)
   {
      ...
      m_strLastRequest.Format("next_grbl_merge_toolmators(\"%s\", \"%s\" %d)", strUser, strSince, (int)bForceAppAuth);
      ...
      if (!bForceAppAuth && (str.is_empty() || v.has_property("errors") && v["errors"].get_count() > 0))
      {
         ...
         return next_grbl_merge_toolmators(strUser, strSince, true);
