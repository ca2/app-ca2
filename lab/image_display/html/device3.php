...
<?php
	...
	$sql = 'SELECT "id", "name", "address" FROM "device" LEFT OUTER JOIN "address_device" ON "device"."id" = "address_device"."device" ORDER BY "name", "address"';
	...
	if($result)
	{
		...
		<tr style="background-color:<? echo $bkcolor; ?>;" >
			<td>
				<section id="<? echo $row[0]; ?>" address="<? echo $row[2]; ?>">
			...
			<td>
				<section address="<? echo $row[2]; ?>">
					<input type="text" contenteditable="true" style="opacity:0.5;" />
				</section>
			</td>
			<td>
				<section address="<? echo $row[2]; ?>">
					<input type="text" fullscreen="true" style="opacity:0.5;" />
				</section>
			</td>
		</tr>
