...
namespace grbl_merge_tool
{
   ...
   void total_progress(struct serial_serial * pserial, int iTotal)
   {
   ...
   bool application::init_instance()
   {
      ...
      m_bMultiPort = file_exists(::dir::home() / ".quasar_science_multi_port");
...
//   }
   ...
   void application::on_request(::create * pcreate)
   {
      ...
      if(Context.file().exists(pathSrc))
      {
         ...
         Context.file().move(pathDst, pathSrc);
   ...
   ::serial::PortInfo application::pick_port()
   {
      ...
      try
      {
         ...
         if (m_portinfoaUpdate.get_size() <= 0 && m_portinfoOverride.port.has_char())
         {

            return m_portinfoOverride;
   ...
   void application::start_update()
   {
      ...
      fork([&]()
      {
         ...
         while(!(m_bCancel && m_iUpdating <= 0) && m_bUpdate && ::thread_get_run())
         {
            ...
            pred_Sleep(1500, [&]() { return !(m_bCancel && m_iUpdating <= 0) && m_bUpdate && ::thread_get_run(); });

            if (m_bCancel && m_iUpdating <= 0)
            {
...
//   }
   ...
   void application::download()
   {
      ...
      fork([&]()
      {
         ...
         m_pathDownload = Context.dir().download() / ::file::path(m_strUrl).name();
...
//      file_put_contents_dup("/bergedge/tst", strCommand);
   ...
   void application::select_serial_and_update(int iIndex)
   {
      ...
      m_portinfoOverride = m_portinfoa[iIndex];

      start_update();

   }

   void application::get_firmware_version_label(char * pszChar, int iSize)
   {
      ...
      strncpy(pszChar, strFirmwareVersion, iSize);
   ...
   void application::manual_com()
   {
      ...
      for (index i = 0; i < m_portinfoa.get_count(); i++)
      {
         ...
         if (m_portinfoOverride.port.has_char())
         {

            if (m_portinfoa[i].port == m_portinfoOverride.port)
            {
   ...
   void application::show_log()
   {
      ...
      Context.os().file_open(get_log_path());
   ...
   void application::check_downloaded_bin()
   {
      ...
      fork([&]()
      {
         ...
         ::file::path pathFolder(Context.dir().download());
         ...
         else
         {
            ...
            m_pathLatestDownloadedBin = Context.dir().download() / ("Rainbow_Update_V" + stra.last() + ".bin");
...
#if defined(LINUX)
      path = ::dir::home() / ".config/grbl_merge_tool/log.txt";
#elif defined(MACOS)
...
aura_app grbl_merge_tool_flags(aura_app::flag_do_not_install);
